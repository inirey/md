...
const menusFolder = global.__dirname(join(__dirname, './menu/index'));
const menusFilter = (filename) => /\.js$/.test(filename);
global.menu = {};
async function filesInit() {
    for (const filename of readdirSync(menusFolder).filter(menusFilter)) {
        try {
            const file = global.__filename(join(menusFolder, filename));
            const module = await import(file);
            global.menu[filename] = module.default || module;
        } catch (e) {
            conn.logger.error(e);
            delete global.menu[filename];
        }
    }
}
filesInit().then((_) => Object.keys(global.menu)).catch(console.error);

global.reload = async (_ev, filename) => {
    if (menusFilter(filename)) {
        const dir = global.__filename(join(menusFolder, filename), true);
        if (filename in global.menu) {
            if (existsSync(dir)) conn.logger.info(` updated menus - '${filename}'`);
            else {
                conn.logger.warn(`deleted menus - '${filename}'`);
                return delete global.menu[filename];
            }
        } else conn.logger.info(`new menus - '${filename}'`);
        ...
        if (err) conn.logger.error(`syntax error while loading '${filename}'\n${format(err)}`);
            ...
            try {
                ...
                conn.logger.error(`error require menus '${filename}\n${format(e)}'`);
            } finally {
                global.menu = Object.fromEntries(Object.entries(global.menu).sort(([a], [b]) => a.localeCompare(b)));
            }
        }
    }
};
Object.freeze(global.reload);
watch(menusFolder, global.reload);
